...
2.At interation t, draw a candidate from proposal function.
3.Compute an acceptance ratio r (probability)
4.Accept the proposal value as the value in interation t with probability min(r,1). If the proposal value is not accepted, then the value in interation t should remain the same.
5.Repeat steps 2-4. 
...
3. For each value P1 and P2 in uniform sample, add new x and y.
$$x=-\frac{1}{y}log(1-p+pe^{-yB})$$
$$y=-\frac{1}{x}log(1-p+pe^{-xB})$$
