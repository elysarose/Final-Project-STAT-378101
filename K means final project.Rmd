...
We can see from the graph that data Irin separated badly. Our clustering function defined only 2 clusters on unscaled data. 
...
accuracy_ratio
```
66.6% the accuracy ratio. Although it gives us a good result, the result from the graph shows us that clustering going badly. 

Now we will use scaled data in order to compare the efficiency and accuracy of the algorithm K means using it on scaled and unscaled data. 
```{r}
data(iris)
...
We can see from the graph that scaled data Irin seems to be well separated in comparison with non-scaled data.We can clearly ovserve three clusters.  
...
accuracy_ratio
```
The accuracy ratio is about 85.3% now, which is much better. This result supports graph, visually the scaled clusters are cleaner than the unscaled data. 
