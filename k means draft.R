...
all_norms1<-function(x, k){
all_norms1 <- matrix(0, nrow=nrow(x), ncol=k+2)
...
#col of all_norms
for(i in 1:nrow(x)) {
  min_dist_index <- which.min(as.vector(all_norms1[i, c(1:k)]))
  all_norms1[i,k+1] <- min_dist_index
}  
return(all_norms1)
}
all_norms1(wine,3)
